...
const getRivalMove = store => next => action => {
    ...
    const { gameId, playerId, playingSide } = store.getState().game;
    const get = () => 
    console.log('GET RIVAL', playingSide , gameId, playerId);
        fetch('http://xo.t.javascript.ninja/move', {
            headers: {
                "Content-Type": "application/json; charset=utf-8",
                "Game-ID": `${gameId}`,
        ...
        .catch(er => {console.log('CATCH ERROR!!!!', er); get();});
